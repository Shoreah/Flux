...
export default function NowShowing({ movies }) {
  ...
  return (
    <div className="flex flex-col justify-center lg:flex-row mx-auto mb-18 px-10 pb-10 pt-6 gap-6">
      <div
        onClick={() => goToDetails(main)}
        className="bg-[#1f1f1f] w-full lg:w-[590px] rounded-xl p-5 hover:shadow-[0_0_30px_rgba(192,38,211,0.15)] cursor-pointer transition duration-300 border border-white/5 flex flex-row lg:flex-col gap-4"
      >
        <div className="w-1/2 lg:w-full shrink-0">
          <motion.img
            layoutId={mounted ? `poster-movie-${main.id}` : undefined}
            src={getImageUrl(main.backdrop_path)}
            alt={main.title}
      ...
      <div className="w-full lg:w-[600px] flex flex-col gap-6 lg:self-stretch">
        <div
          ...
          className="bg-[#1f1f1f] flex-1 rounded-xl px-5 py-5 flex items-center gap-4 hover:shadow-[0_0_30px_rgba(192,38,211,0.15)] cursor-pointer transition duration-300 border border-white/5"
        >
          ...
          <div className="w-1/2 shrink-0">
            <motion.img
              ...
              src={getImageUrl(second.backdrop_path)}
              alt={second.title}
        ...
        >
          <div className="flex flex-col gap-2 w-1/2 min-w-0">
            ...
            <button
              ...
              className="bg-[#C026D3] hover:bg-[#a21caf] rounded-full py-2 px-5 text-white text-sm font-bold transition duration-300 w-fit mt-1"
            >
              Learn More
            </button>
          </div>
          <div className="w-1/2 shrink-0">
            <motion.img
              ...
              src={getImageUrl(third.backdrop_path)}
              alt={third.title}
              className="h-44 w-full object-cover rounded-lg"
            />
          </div>
